use crate::arch::{
    pa2kva, pa2pte, pte2pa, ArchPageTableEntryTrait, ArchTrait, MACHINE_SIZE, PAGE_SHIFT,
    PAGE_SIZE, PAGE_TABLE_L1_SHIFT, PAGE_TABLE_L2_SHIFT, PAGE_TABLE_L3_SHIFT,
};
use crate::lib::page_table::{
...
use crate::config::{
    CONFIG_READ_ONLY_LEVEL_1_PAGE_TABLE_BTM, CONFIG_READ_ONLY_LEVEL_2_PAGE_TABLE_BTM,
    CONFIG_READ_ONLY_LEVEL_3_PAGE_TABLE_BTM,
};
...
impl PageTableTrait for Riscv64PageTable {
    fn new(directory: PageFrame) -> Self {
        let r = Riscv64PageTable { directory };
    ...
    fn map(&self, va: usize, pa: usize, attr: EntryAttribute) {
        ...
        if !l1e.valid() {
            ...
            if va <= CONFIG_READ_ONLY_LEVEL_1_PAGE_TABLE_BTM {
                self.map(
                    CONFIG_READ_ONLY_LEVEL_2_PAGE_TABLE_BTM + va.l1x() * PAGE_SIZE,
                    pte2pa(l1e.to_usize()),
    ...
    fn recursive_map(&self, _va: usize) {
        self.map(
            CONFIG_READ_ONLY_LEVEL_1_PAGE_TABLE_BTM,
            self.directory.pa(),
            EntryAttribute::user_readonly(),
        );
    }
