...
impl<T> core::convert::From<Result<T, super::syscall::Error>> for SystemCallResult
where
    T: SystemCallResultOk,
{
    fn from(sce: Result<T, super::syscall::Error>) -> Self {
        SystemCallResult::R(match sce {
...
impl InterruptServiceRoutine for Isr {
    fn system_call() {
        ...
        unsafe {
            ...
            let scr =
                match (*crate::arch::Arch::context()).syscall_number() {
                    1 => {
                    ...
                    2 => SystemCall::getpid().into(),
                    3 => SystemCall::process_yield().into(),
                    4 => SystemCall::process_destroy(arg(0) as u16).into(),
                    5 => SystemCall::process_set_exception_handler(arg(0) as u16, arg(1), arg(2))
                        .into(),
                    6 => SystemCall::mem_alloc(arg(0) as u16, arg(1), arg(2)).into(),
                    7 => SystemCall::mem_map(arg(0) as u16, arg(1), arg(2) as u16, arg(3), arg(4))
                        .into(),
                    8 => SystemCall::mem_unmap(arg(0) as u16, arg(1)).into(),
                    9 => SystemCall::process_alloc().into(),
                    10 => {
                        use crate::lib::process::Status::{PsFree, PsNotRunnable, PsRunnable};
                        let ps = match arg(1) {
                            1 => PsRunnable,
                            2 => PsNotRunnable,
                            _ => PsFree,
                    ...
                    11 => SystemCall::ipc_receive(arg(0)).into(),
                    12 => SystemCall::ipc_can_send(arg(0) as u16, arg(1), arg(2), arg(3)).into(),
            ...
            match scr {
                ...
                SystemCallResult::R(o) => {
                    ...
                    match o {
                        None => {
                            (*crate::arch::Arch::context()).set_syscall_return_value(0);
                        }
                        Some(i) => {
                            (*crate::arch::Arch::context()).set_syscall_return_value(i as usize);
                        }
                    }
                }
            }
            ...
            match page_table.lookup_page(stack_btm) {
                Some(stack_pte) => {
                    ...
                    core::intrinsics::volatile_copy_memory(
                        (stack_frame.kva() + PAGE_SIZE - size_of::<ContextFrame>())
                            as *mut ContextFrame,
    ...
    fn default() {
        unsafe {
            ...
            println!(
                "{:?}",
                (*crate::arch::Arch::running_process().unwrap().pcb())
                    .page_table
                    .unwrap()
                    .lookup_page(crate::Arch::fault_address())
            );
            println!(
                "pc: {:016x}",
                (*crate::arch::Arch::context()).exception_pc()
            );
            println!(
                "{:?}",
                (*crate::arch::Arch::running_process().unwrap().pcb())
                    .page_table
                    .unwrap()
                    .lookup_page((*crate::arch::Arch::context()).exception_pc())
            );
