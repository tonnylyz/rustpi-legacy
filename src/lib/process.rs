...
impl Display for ControlBlock {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), core::fmt::Error> {
        ...
        writeln!(
            f,
            "directory: {:08x}",
            self.page_table.unwrap().directory().pa()
        )?;
        writeln!(f, "context:\n{}", self.context.unwrap())?;
...
impl Process {
    ...
    pub fn pcb(&self) -> *mut ControlBlock {
        unsafe { (&mut PCB_LIST[self.index()]) as *mut ControlBlock }
    }

    pub fn ipc(&self) -> *mut Ipc {
        unsafe { (&mut IPC_LIST[self.index()]) as *mut Ipc }
    }

    pub fn parent(&self) -> Option<Process> {
        unsafe { (*self.pcb()).parent }
    }
    ...
    pub fn setup_vm(&self) {
        unsafe {
            ...
            for i in 0..(CONFIG_PROCESS_IPC_SIZE * CONFIG_PROCESS_NUMBER / PAGE_SIZE) {
                ...
                let pa = kva2pa(
                    &IPC_LIST[i * (PAGE_SIZE / CONFIG_PROCESS_IPC_SIZE)] as *const Ipc as usize,
                );
    ...
    fn load_image(&self, elf: &'static [u8]) {
        unsafe {
            ...
            match page_table.insert_page(
                CONFIG_USER_STACK_TOP - PAGE_SIZE,
                crate::mm::page_pool::alloc(),
    ...
    pub fn run(&self) {
        unsafe {
            ...
            crate::arch::Arch::set_user_page_table(
                (*self.pcb()).page_table.unwrap(),
                self.pid as AddressSpaceId,
            );
    ...
    pub fn is_runnable(&self) -> bool {
        unsafe { (*self.pcb()).status == Status::PsRunnable }
    }
    ...
    pub fn unlock(&self) {
        unsafe { (*self.pcb()).lock.force_unlock() }
    }

    pub fn running_at(&self) -> Option<usize> {
        unsafe { (*self.pcb()).running_at }
    }
