...
fn clear_bss() {
    ...
    unsafe {
        core::intrinsics::volatile_set_memory(start as *mut u8, 0, end - start);
    }
}
...
fn static_check() {
    ...
    unsafe {
        assert_eq!(
            size_of::<crate::lib::process::Ipc>(),
            CONFIG_PROCESS_IPC_SIZE
        );
...
pub fn main() -> ! {
    ...
    if core_id != 0 {
        loop {
            arch::Arch::wait_for_event();
        } // capture other cores here
    }
    clear_bss();
    driver::uart::init();
    static_check();
